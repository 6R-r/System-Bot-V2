...
setInterval(() => {
  http.get(`http://systemtst.glitch.me/`);
}, 280000);
...
const youtube = new YouTube("AIzaSyCtReXk2BcDTDd3DawAGyzZTqse-K79b0g"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyCtReXk2BcDTDd3DawAGyzZTqse-K79b0g"; ///تعديل اساسي سوي اي بي اي جديد
...
const developers = ['441225766781452288'];
...
client.on('message', message => {
    if (message.content.startsWith(prefix + 'sug')) {
        ...
        let Room = message.guild.channels.find(`name`, "↬│・إقتراحات")
        if (!Room) return message.channel.send("Can't find ↬│・إقتراحات channel.").then(d => d.react('❌'))
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@441225766781452288>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`${prefix}avt\` :يعرض لك صورت  اي شخص عن طريق الايدي حتى لو كان مش بالسيرفر 
...
\`${prefix}bc\` : برودكاست
\`${prefix}obc\` : رودكاست للاونلاين
\`${prefix}rolebc\` : برودكاست لرتبة
...
\`${prefix}moveall\` : سحب الكل الى رومك
...
\`${prefix}sb$\` : لإظهار جميع بوتات السيرفر
\`${prefix}role\` : لاعطاء شخص رتبة
\`${prefix}role all\` : لـ إعطاء الجميع رتبة معينة
\`${prefix}role humans\` : اعطاء الرتبة للاشخاص و ليس البوتات
\`${prefix}role bots\` : اعطاء الرتبة للبوتات
\`${prefix}bans\` : يعرض لك عدد المبندين من السيرفر 
\`\`اوامر التقديم\`\` :pencil: 
امر تقديم -تقديم
يجب عمل روم تقديمات
يجب عمل روم قبول و رفض
مثال: \`\`${prefix}قبول @منشن عضو \`\`
لرفض عضو : ${prefix}رفض
مثال: \`\`${prefix}رفض @منشن عضو لست متفاعل بشكل كافِ\`\`


يستحسن استعمال بوت قيف اواي ثاني لأن هاذا يستعمل نظام الدقائق فقط
